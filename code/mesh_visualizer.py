...
Usage:
    python mesh_visualizer.py --input_dir pose2mesh_input/ --output_dir data/output/mesh_frames/ --make_video
...
def visualize_mesh_npy(npy_path, output_path, faces=None, elev=30, azim=45):
    data = np.load(npy_path, allow_pickle=True).item() if np.load(npy_path).dtype == object else np.load(npy_path)
    if isinstance(data, dict):
        verts = data.get('verts') or data.get('vertices') or data.get('v')
...
def main():
    ...
    faces = np.load(args.faces) if args.faces else None
